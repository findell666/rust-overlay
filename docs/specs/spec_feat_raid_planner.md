...
**Decision (resolved): the collection tool is a global hotkey inside the main overlay app**, not a standalone tool — the overlay is running next to the game anyway, and this reuses its capture machinery:

* Dev builds only: the hotkey is registered behind an `app.isPackaged` gate, so packaged installs never get it. No settings UI; the keys are edited by hand in `config.json`.
* Config keys: `captureHotkey` (default `'F7'`, `''` disables) and `captureDir` (`null` → `<repo>/captures/raid` when running unpacked; an absolute path overrides, relative paths are ignored with a warning).
* On keypress the main process grabs the screen through the exact same path as the recycler calculation (overlay hidden, `captureDelayMs` wait, `desktopCapturer` PNG of the primary display) and writes it as `raid-<timestamp>.png`. The user renames the batch to the expected-value convention (`wall-stone_320_500.png`, `negative_01.png`) afterwards, instead of switching out of the game between shots.
* A transient toast at the top of the overlay confirms each save and flashes longer on failure — a lost save must not look like a lost keypress.